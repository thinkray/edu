...
class BookingListAPI(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            crash_booking = Booking.objects.filter(Q(start_date__gte=cleaned_data['start_date'], start_date__lt=cleaned_data['end_date']) | Q(end_date__gte=cleaned_data['end_date'], end_date__lt=cleaned_data['end_date']))
            ...
            booking = Booking(course=cleaned_data['course'], start_date=cleaned_data['start_date'],
                              end_date=cleaned_data['end_date'], teacher=cleaned_data['teacher'], is_full=False, info=cleaned_data['info'])
            booking.save()
            return JsonResponse({
                'status': 200,
                'message': 'Success'
